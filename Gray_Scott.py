...
import timehttps://github.com/lukaswittmann/Gray-Scott_Model
import random
...
plt.ion()
figure = plt.imshow(A[:,:], cmap='inferno',interpolation="bilinear")  # , cmap='plasma', interpolation="nearest", cmap='hsv', interpolation="lanczos"
plt.figsize = (10,10)
#plt.title("Bereit..")
plt.axis('off')
plt.clim(0, 0.3)
...
for i in range(1, n):
    ...
    A_diff_gesamt[:, :] = Da * (- 4 * A[:, :] + A_oben[:, :] + A_unten[:, :] + A_links[:, :] + A_rechts[:, :])
    B_diff_gesamt[:, :] = Db * (- 4 * B[:, :] + B_oben[:, :] + B_unten[:, :] + B_links[:, :] + B_rechts[:, :])

    An[:,:] = A[:,:] + (- (A[:,:] * B[:,:] ** 2) + f * (1 - A[:,:])) * dt + A_diff_gesamt[:, :] * dt
    Bn[:,:] = B[:,:] + (+ (A[:, :] * B[:, :] ** 2) - ((k + f) * B[:,:])) * dt + B_diff_gesamt[:, :] * dt
